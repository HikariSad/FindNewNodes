...
class SubConvert():
    ...
    def url_ss_to_json(oldonenode):
        try:
            ...
            if (onenode.find('@') > -1):
                nod = onenode.split('@', 1)  # 第二个参数为 1，返回两个参数列表
                onenode = base64.b64decode(StrText.get_str_base64(nod[0]).encode('utf-8')).decode('utf-8') + '@' + nod[
                    1]
            else:
                onenode = base64.b64decode(StrText.get_str_base64(nod[0]).encode('utf-8')).decode('utf-8')
            if (onenode.find('ss://') == 0 and onenode.find('@') > -1):
                nod = onenode.split('@', 1)  # 第二个参数为 1，返回两个参数列表
                onenode = base64.b64decode(StrText.get_str_base64(nod[0][5:]).encode('utf-8')).decode('utf-8') + '@' + \
                          nod[1]
            ...
            # 字典格式SS信息
            onenode = '{"cipher":"' + cipher + '","password":"' + password + '","server":"' + server + '","port":"' + str(
                port) + '","name":"' + name + '"}'
            ...
            LocalFile.write_LogFile(
                'SubConvert-Line-193-Exception:' + str(ex) + '\nonenode:' + onenode + '\noldonenode:' + oldonenode)
            return ''
    ...
    def url_trojan_to_json(j):
        try:
            ...
            if (port.isnumeric() and password != '' and password != 'null'):
                onenode = '{' + '"name": "' + newname + '", "type": "trojan", "server": "' + server + '", "port": "' + str(
    ...
    def clash_to_all_url(oldonenode):
        try:
            ...
            node = json.loads(onenode)
            if (node['type'] == 'ss'):
                onenode = 'ss://' + base64.b64encode(
                    (node['cipher'] + ':' + node['password'] + '@' + node['server'] + ':' + str(node['port'])).encode(
                        "utf-8")).decode("utf-8") + '#' + node['name']
            elif (node['type'] == 'trojan'):
                onenode = 'trojan://' + node['password'] + '@' + node['server'] + ':' + str(node['port']) + '#' + node[
                    'name']
                ...
                onenode = node['server'] + ':' + node['port'] + ':' + node['protocol'] + ':' + node['cipher'] + ':' + \
                          node['obfs'] + ':' + base64.b64encode(node['password'].encode("utf-8")).decode("utf-8")
    ...
    def surge_to_all_url(onenode):
        try:
            ...
            if (node['type'] == 'ss'):
                onenode = 'ss://' + base64.b64encode(
                    (node['cipher'] + ':' + node['password'] + '@' + node['server'] + ':' + node['port']).encode(
                        "utf-8")).decode("utf-8") + '#' + node['name']
            elif (node['type'] == 'trojan'):
                onenode = 'trojan://' + node['password'] + '@' + node['server'] + ':' + node['port'] + '#' + node[
                    'name']
            elif (node['type'] == 'vmess'):
